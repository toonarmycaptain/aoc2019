...
parameter is 50, its value is the value stored at address 50 in memory. Until now, all parameters have been in position mode.
...
- its value is the value stored at address 4 (33). The second parameter, 3 in immediate mode, simply has value 3. The result of this operation, 33 * 3 = 99, is written according to the third parameter, 4 in position mode, which also works like it did before - 99 is written to address 4.
...
was from the expected value, where 0 means the test was successful. Non-zero outputs mean that a function is not working correctly; check the instructions that were run before the output instruction to see which one failed.

Finally, the program will output a diagnostic code and immediately halt. This final output isn't an error; an output
followed immediately by a halt means the program finished. If all outputs were zero except the diagnostic code, the diagnostic program ran successfully.
...
from dec_2nd.dec_2nd_solution_part_1 import Computer
...
class TESTComputer():
    ...
    def run_program(self):
        ...
        Also modify to parse modes in opcode: ?????
        :return:
        """
        try:
            pointer_index = 0
            while True:
                opcode = self._intcode_program[pointer_index]
    ...
    def addiction_opcode(self, position: int):
        ...
        arg_index_a, arg_index_b, target = self._intcode_program[position + 1:position + 4]
        self._intcode_program[target] = self._intcode_program[arg_index_a] + self._intcode_program[arg_index_b]
        return position + 4
    ...
    def output_opcode(self, position: int):
        output_source = self._intcode_program[position + 1]
        self.outputs.append(self._intcode_program[output_source])
        return position + 2
