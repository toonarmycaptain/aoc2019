"""Advent of Code solution December 2nd part 1
...
def parse_puzzle_input() -> List[int]:
    ...
    with open(Path(problem_folder, 'dec_2nd_puzzle_input.txt'), 'r') as input_file:
...
class Computer:
    ...
    def run_program(self):
        try:
            opcode_index = 0
            while True:
                opcode = self._intcode_program[opcode_index]
                self.opcode_switch[opcode](opcode_index)
                opcode_index += 4
    ...
    def addiction_opcode(self, position: int):
        """Addition opcode."""
        self.validate_opcode_position(position)
    ...
    @staticmethod
    def validate_opcode_position(position: int):
        if position % 4:
            raise ValueError('Position passed is not an opcode because '
                             f'position {position} is not a multiple of 4.')


def fix_program(position_1_replacement: int = 12, position_2_replacement: int = 2) -> int:
    """
    Solve day 1: default args: returns 6087827
    ...
    # Fix program:
    puzzle_input[1] = position_1_replacement
    puzzle_input[2] = position_2_replacement

    # Run program:
    c = Computer(puzzle_input)
    return c.run_program()[0]
