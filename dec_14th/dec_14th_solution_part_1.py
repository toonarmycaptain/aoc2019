"""Advent of Code solution December 14th part 1
...
def parse_puzzle_input(file=None) -> Dict[str, Tuple[int, Dict[str, int]]]:
    output = {}
    problem_folder = Path(__file__).parent
    if file is None:
        file = 'dec_14th_puzzle_input.txt'
    # with open(Path(problem_folder, 'dec_14th_puzzle_input.txt'), 'r') as input_file:
    with open(Path(problem_folder, file), 'r') as input_file:
        for line in input_file:
            reactants, product = line.split(' => ')
            product_quantity, product_name = product.split()
            reactants_dict = {reactant_name: int(reactant_quantity) for reactant in reactants.split(', ') for
                              reactant_quantity, reactant_name in [reactant.split()]}
            output[product_name] = (int(product_quantity), reactants_dict)
    return output


def find_ore_needed_for_fuel(equations: Dict[str, Tuple[int, Dict[str, int]]], fuel_quantity_needed: int):
    needed_resources = {'FUEL': fuel_quantity_needed}
    unused_reactants: dict = defaultdict(int)  # Equivalent of defaultdict(lambda: int())
    while needed_resources.keys() != {'ORE'}:
        needed_next: dict = defaultdict(int)
        for product, product_quantity in needed_resources.items():
            if product == 'ORE':
                needed_next['ORE'] += product_quantity
                continue
            reactants_multiple, reactants = equations[product]
            # Get quantity of each reactant needed for product_quantity of product:
            multiple = math.ceil(product_quantity / reactants_multiple)  # Round up
            for reactant_name, reactant_quantity in reactants.items():
                needed_next[reactant_name] += reactant_quantity * multiple
            # Store leftover reactants
            unused_reactants[product] += multiple * reactants_multiple - product_quantity

        # Remove any used reactants from leftover reactants pool
        for unused_reactant_name, unused_reactant_quantity in unused_reactants.items():
            quantity_used = min(unused_reactant_quantity, needed_next[unused_reactant_name])
            needed_next[unused_reactant_name] -= quantity_used
            unused_reactants[unused_reactant_name] -= quantity_used

        needed_resources = {product_name: product_quantity for product_name, product_quantity in needed_next.items() if product_quantity}

    return needed_resources['ORE']


def solve_part_1(equations: Dict[str, Tuple[int, Dict[str, int]]]):
    return find_ore_needed_for_fuel(equations, 1)


"""
solve_part_1(parse_puzzle_input()) = 628586
"""
