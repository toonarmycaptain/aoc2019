"""Advent of Code solution December 13th part 1
...
def solve_part_1(intcode_program: List[int]):
    arcade_game = ArcadeComputer(intcode_program)
    arcade_game.run_program()
    screen_data: Dict = defaultdict(lambda: 0)
    for output_index in range(0, len(arcade_game.outputs[:-2]), 3):
        xy = (arcade_game.outputs[output_index], arcade_game.outputs[output_index + 1])
        tile = arcade_game.outputs[output_index + 2]
        screen_data[xy] = tile
    return Counter(screen_data.values())[2]


"""
solve_part_1(parse_puzzle_input())) = 291
"""
