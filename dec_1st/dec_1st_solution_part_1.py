"""Advent of Code solution December 1st
...
def parse_puzzle_input() -> List[int]:
    ...
    with open(Path(problem_folder, 'dec_1st_puzzle_input.txt'), 'r') as input_file:
...
def mass_fuel_req(module_mass: int) -> int:
    fuel = module_mass//3 - 2
    return fuel if fuel > 0 else 0


def sum_of_fuel_requirements(module_masses: List[int]) -> int:
    fuel_reqs = [mass_fuel_req(module_mass) for module_mass in module_masses]
    return sum(fuel_reqs)

"""
sum_of_fuel_requirements(PUZZLE_INPUT) = 3318604
"""
